    ...
    def forward(self, x: torch.Tensor, img_size: int) -> torch.Tensor:
        ...
        if not self.training:  # inference
            if self.grid.shape[2:4] != x.shape[2:4]:
                self.grid = self._make_grid(nx, ny).to(
                    x.device
                )  # Tạo ra lưới, trong đó mỗi phần tử chứa tọa độ của cell đó so với ảnh thật
            ...
            if self.new_coords:
                ...
                x[..., 2:4] = (
                    torch.exp(x[..., 2:4]) * self.anchor_grid
                )  # wh # anchor_grid là lưới lưu lại tọa độ thực tế của anchor -> nhân lên để tính tọa độ w và h thực tế
...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        return (
            yolo_outputs if self.training else torch.cat(yolo_outputs, 1)
        )  # với training, trả về các bb theo từng anchors để tính loss, nhưng lúc inference thì concat lại hết để trả về làm prediction
